...
Hooks.on("renderChatMessageHTML", (message, html) => {
    ...
    $button.on("click", async (ev) => {
        const token = canvas.tokens.ownedTokens.find(
        ...
        const skillObject = RMUSkillParser.getRawSkillById(
            token.actor,
            skillUuid,
        );
        ...
        if (!skillObject) {
            ui.notifications.error(
                game.i18n.localize("RMU_CS.Notifications.SkillNotFound"),
            );
            return;
        }
        ...
        if (game.system?.api?.rmuTokenSkillAction) {
            game.system.api.rmuTokenSkillAction(
                token,
                skillObject,
                maneuverOptions,
            );
            ...
            ui.notifications.error(
                game.i18n.localize("RMU_CS.Notifications.ApiNotFound"),
            );
        }
    });
});
        ...
        tokenControls.tools["rmu-complementary-skills"] = {
            ...
            onChange: () => {
                ...
                if (!game.rmuComplementarySkills?.LauncherApp) {
                    ...
                    ui.notifications.error(
                        game.i18n.localize(
                            "RMU_CS.Notifications.NotInitialized",
                        ),
                    );
                    return;
                }
                ...
                // Filter tokens to only include those with valid actors
                const validTokens = controlledTokens.filter(
                    (t) => t.actor && VALID_ACTOR_TYPES.includes(t.actor.type),
                );

                // Ensure at least one VALID token is selected.
                if (validTokens.length === 0) {
                    ui.notifications.warn(
                        game.i18n.localize("RMU_CS.Notifications.SelectOne"),
                    );
                    return;
                }

                // Open the launcher application with the selected tokens.
                new game.rmuComplementarySkills.LauncherApp(validTokens).render(
                    true,
                );
            },
            button: true,
